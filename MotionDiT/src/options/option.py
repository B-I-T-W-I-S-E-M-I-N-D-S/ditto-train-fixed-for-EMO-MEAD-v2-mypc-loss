...
class TrainOptions(PrintableConfig):
    ...
    use_emo2vec: bool = False    # use Emotion2vec audio emotion features (MEMO integration)
    ...
    emotion_feat_dim: int = 0           # Emotion2vec feature dim (0 = disabled)
    num_emotion_tokens: int = 4         # emotion cross-attn tokens injected into MotionDecoder

    ########## emotion auxiliary losses ##########
    use_emotion_losses: bool = False      # master switch (off by default; safe for existing runs)
    lambda_emo_cls: float = 0.10         # weight for emotion classification loss
    lambda_emo_perc: float = 0.05        # weight for perceptual emotion consistency loss
    lambda_emo_contrast: float = 0.05    # weight for contrastive emotion loss
    lambda_emo_temporal: float = 0.02    # weight for temporal emotion consistency loss
