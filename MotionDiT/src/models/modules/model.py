...
from .emotion_proj import EmotionProjection
...
class MotionDecoder(nn.Module):
    def __init__(
        ...
        emotion_feat_dim: int = 0,       # 0 = disabled (backward compat)
        num_emotion_tokens: int = 4,     # emotion tokens injected into cross-attn
        **kwargs
    ) -> None:
        ...
        # ── Emotion2vec projection (optional) ──────────────────────────────
        self.emotion_proj = None
        if emotion_feat_dim > 0:
    ...
    def forward(
        ...
        # ── Inject Emotion2vec tokens (if enabled) ─────────────────────────
        if self.emotion_proj is not None and emotion_embed is not None:
            # emotion_embed: [B, L, emotion_feat_dim]  →  [B, num_emotion_tokens, latent_dim]
            emo_tokens = self.emotion_proj(
                emotion_embed.to(self.cond_projection.weight.dtype)
            )
            c = torch.cat((c, emo_tokens), dim=-2)
        # ──────────────────────────────────────────────────────────────────
